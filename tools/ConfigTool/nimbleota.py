...
async def update_device(client: BleakClient, file_size: int, sectors: list):
    success = False
    queue = asyncio.Queue()
    await client.start_notify(
        OTA_COMMAND_UUID,
        lambda sender, data: asyncio.create_task(cmd_notification_handler(sender, data, queue)),
    )
    ...
    if ack == ACK_ACCEPTED:
        await client.start_notify(
            OTA_FIRMWARE_UUID,
            lambda sender, data: asyncio.create_task(fw_notification_handler(sender, data, queue)),
        )
        ...
        while sec_idx < sec_count:
            ...
            print(f"Sending sector {sec_idx}/{sec_count-1}, progress: {round(sec_idx/(sec_count-1)*100,1)} %")
            await upload_sector(
...
async def perform_ota_update(client: BleakClient, file_name: str):
    ...
    return await update_device(client, file_size, sectors)
