...
class ConfigToolApp:
    ...
    def __init__(self, root):
        ...
        self.clear_btn.grid(row=5, column=0, padx=5, pady=10, sticky="w")

        # Show selection section
        self.cmd_file_path_var = tk.StringVar()
        tk.Label(root, text="Command File:").grid(row=6, column=0, padx=5, pady=5, sticky="w")

        self.cmd_file_entry = tk.Entry(root, textvariable=self.cmd_file_path_var)
        self.cmd_file_entry.grid(row=6, column=1, padx=5, pady=5, sticky="ew")

        self.cmd_browse_btn = tk.Button(root, text="Browse", command=self.browse_cmd_file)
        self.cmd_browse_btn.grid(row=6, column=2, padx=5, pady=5, sticky="ew")

        self.cmd_load_btn = tk.Button(root, text="Load", command=self.load_cmd)
        self.cmd_load_btn.grid(row=6, column=3, padx=5, pady=5, sticky="ew")

        self.cmd_send_btn = tk.Button(root, text="Send", width=12, command=self.send_cmd, state="disabled")
        self.cmd_send_btn.grid(row=7, column=2, padx=5, pady=10, sticky="e")

        self.stop_show_btn = tk.Button(root, text="Stop Show", width=12, command=self.stop_show, state="disabled")
        self.stop_show_btn.grid(row=7, column=3, padx=5, pady=10, sticky="e")
    ...
    def update_buttons(self, action_ongoing: bool = False):
        ...
        if self.state.connected:
            ...
            self.stop_show_btn.config(state="normal")
